...
//
// Modified 2021 by Aaron Fontaine:
//  - Added wifi_prov_mgr_reset_to_ready_state() function.
//
...
esp_err_t wifi_prov_mgr_reset_to_ready_state(void)
{
    if (!prov_ctx_lock) {
        ESP_LOGE(TAG, "Provisioning manager not initialized");
        return ESP_ERR_INVALID_STATE;
    ...
    if (WIFI_PROV_STATE_STARTED == prov_ctx->prov_state) {
        // Already in the STARTED state. Nothing to do.
        RELEASE_LOCK(prov_ctx_lock);
        return ESP_OK;
    }

    if ((WIFI_PROV_STATE_FAIL != prov_ctx->prov_state) &&
        (WIFI_PROV_STATE_SUCCESS != prov_ctx->prov_state)) {
        // Cannot reset while in middle of operation.
        RELEASE_LOCK(prov_ctx_lock);
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set Wi-Fi storage to FLASH");
        RELEASE_LOCK(prov_ctx_lock);
        return err;
    }
    esp_wifi_set_config(ESP_IF_WIFI_STA, &wifi_cfg_empty);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to clear Wi-Fi credentials from FLAHS NVS");
        RELEASE_LOCK(prov_ctx_lock);
        return err;
    }
