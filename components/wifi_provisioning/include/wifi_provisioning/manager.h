...
/**
 * @brief   Resets the provisioning manager back to the ready state.
 *
 * Deletes the credentials saved for STA mode, switches STA mode config back to
 * RAM with an empty config, and transitions back to STARTED state in preperation
 * for more commands on the prov-scan and prov-config endpoints.
 *
 * @return
 *  - ESP_OK      : Provisioning service reset successfully or was already in
 *                  Started state
 *  - ESP_FAIL    : Failed to reset provisioning service
 *  - ESP_ERR_INVALID_STATE : Provisioning manager not initialized or not in FAIL
 *                  or SUCCESS state. This error will occur if still in STARTING
 *                  state or if credentials have been received but not yet resulted
 *                  in CRED_FAIL or CRED_SUCCESS events.
 */
